^XA
    ^FX Return Address
    ^FO30,30^A0,25^FDMeinhart Solutions LLC^FS
    ^FO30,60^A0,25^FD800 S Henrietta St^FS
    ^FO30,90^A0,25^FDEffingham, IL 62401^FS
    ...
    ^FX Separator
    ^FO30,300^GB740,1,1^FS

    ^FX Set barbode width
    ^BY3

    ^FX Centered code 128 barcode for tracking number
    ^FO50,720^BCN,125^FD1Z6V519Y5555555555^FS

    ^FX Centered code 128 barcode for reference number
    ^FO100,1070^BCN,75,Y,Y^FDDtAAAKx5AAAA^FS

    ^FX QR Code
    ^FO30,305^A0,25^FDAmazon QR Code^FS
    ^FO30,325^BQN,2,4,A^FDMA,1Z6V519Y5555555555|DtAAAKx5AAAA|E1|LAS2 01 AMAZON RETURNS CR|3837 Bay Lake Trail|Ste 113|89193|Las Vegas|NV|AMAZON RETURNS|STORE ADDRESS FIRST LINE||98144|Seattle|WA||DtAAAKx5AAAA|||^FS
    ...
    ^FO150,660^AE,25^FD1Z R7X 555 55 5555 5555^FS
    ...
    ^FX Reference number
    ^FO240,980^AE^FDDtAAAK x5AAAA^FS

    ^FX Thin bottom separator
    ^FO30,1020^GB740,2,1^FS

    ^FX E code
    ^FO660,1090^A0,50^FDE1^FS
